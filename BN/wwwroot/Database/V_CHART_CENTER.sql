...
ALTER   VIEW [dbo].[V_CHART_CENTER] as 
	...
	FROM (SELECT tcr.emp_no FROM tr_course_registration tcr
	left join tb_employee tr on tcr.emp_no = tr.emp_no
	where YEAR(register_at) = YEAR(GETDATE())
	and (last_status = 'Approved') and tr.dept_abb = te.dept_abb GROUP BY left(course_no, 7),tcr.emp_no) as tb) as total
	FROM tr_course_registration tcr
	left join tb_employee te on tcr.emp_no = te.emp_no
	where YEAR(register_at) = YEAR(GETDATE())
	and (last_status = 'Approved')
